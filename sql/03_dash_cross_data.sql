-- DASH_CROSS_DATA: cruzamentos dimensionais por política/safra
CREATE OR REPLACE TABLE `meli-bi-data.SBOX_CREDITSTC.DASH_CROSS_DATA` AS
WITH base AS (
  SELECT
    ...
    CASE
      ...
      ELSE NULL
    END AS politica,
    IFNULL(RANGE_BUREAU,   'N/D') AS rb,
    IFNULL(RATING_TC,      'S/R') AS rt,
    IFNULL(FLAG_APP_ATIVO, 'N/D') AS fa,
    IFNULL(FLAG_SELLERS,   'N/D') AS fs,
    IFNULL(NISE_TAG,       'N/D') AS nt,
    ...
    DEUDA_OVER6_MOB1, DEUDA_MOB1,
    DEUDA_OVER15_MOB2, DEUDA_MOB2,
...
SELECT
  safra, segmento, politica,
  dim1_nome, dim1_val, dim2_nome, dim2_val,
  COUNT(*)                                                              AS cl,
...
FROM base
CROSS JOIN UNNEST([
  STRUCT('RANGE_BUREAU' AS dim1_nome, rb AS dim1_val, 'RATING_TC'      AS dim2_nome, rt AS dim2_val),
  STRUCT('RANGE_BUREAU',              rb,              'FLAG_APP_ATIVO',              fa),
  STRUCT('RANGE_BUREAU',              rb,              'FLAG_SELLERS',                fs),
  STRUCT('RANGE_BUREAU',              rb,              'NISE_TAG',                    nt),
  STRUCT('RATING_TC',                 rt,              'FLAG_APP_ATIVO',              fa),
  STRUCT('RATING_TC',                 rt,              'FLAG_SELLERS',                fs),
  STRUCT('RATING_TC',                 rt,              'NISE_TAG',                    nt),
  STRUCT('FLAG_APP_ATIVO',            fa,              'FLAG_SELLERS',                fs),
  STRUCT('FLAG_APP_ATIVO',            fa,              'NISE_TAG',                    nt),
  STRUCT('FLAG_SELLERS',              fs,              'NISE_TAG',                    nt)
]) AS pairs
WHERE politica IS NOT NULL
GROUP BY ALL
ORDER BY safra, segmento, politica, dim1_nome, dim1_val, dim2_nome, dim2_val
