-- DASH_CANAL_DIM_DATA: breakdowns dimensionais por canal de aquisição
CREATE OR REPLACE TABLE `meli-bi-data.SBOX_CREDITSTC.DASH_CANAL_DIM_DATA` AS
WITH congrats AS (
...
SELECT
  safra, segmento, canal, dim_nome, dim_val,
  COUNT(*)                                                              AS cl,
...
FROM base
CROSS JOIN UNNEST([
  STRUCT('RANGE_BUREAU'   AS dim_nome, rb AS dim_val),
  STRUCT('RATING_TC',                  rt),
  STRUCT('FLAG_APP_ATIVO',             fa),
  STRUCT('FLAG_SELLERS',               fs),
  STRUCT('NISE_TAG',                   nt)
]) AS dims
GROUP BY ALL
ORDER BY safra, segmento, canal, dim_nome, dim_val
